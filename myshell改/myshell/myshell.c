#include "myshell.h"
char* INS[] = { "cd", "clr", "dir", "environ", "echo", "help", "quit", "myshell", "set", "time", "shift","history","jobs" };//指令集合
...
history              output all the instruction in history                      \n\
	...
	write(tempfd, helpout, sizeof(helpout));
	if (fork() == 0)
	{
...
void Initial(char * ShellPath)//主要环境变量包括{ "HOME", "COMP", "ACOMP", "SHELL","HOST", "USER", "PATH", "PWD" }
{
	...
	for (i = 0; i<256; i++)
	{
		if (path[i] == ':')
		{
			...
			while (dirP!=NULL)
			{
				if ((dirP->d_type == DT_REG) && isalpha(*(dirP->d_name)))
				{
					WORD tmp;
					strcpy(tmp, dirP->d_name);
					treeinsert(tmp, pathP, 0);
				}
				dirP = readdir(dirSS);
			}
			closedir(dirSS);
...
void analyInput(int argc, char *argv[PARAMETER])//分析分割后的输入字符串
{
	...
	{   
		...
		else if (*(argv[i]) == '<')// 从文件读取输入(文件名必须为>后面那个参数)
		{
			...
			fp = fopen(argv[++i], "r");
			if (fp == NULL || fclose(fp) == -1|| !fread(buf, 1, sizeof(buf), fp) )
				return;
			inputCommand(buf, myargc, myargv);

			write(tempfd, buf, 1024);
			memset(myargv, 0, sizeof(char*)*PARAMETER);
			myargc = 0;
		}
		else if (*(argv[i]) == '>')
		{                    
			...
			ouputCommand(buf, myargc, myargv);

			write(tempfd, buf, 1024);
			memset(myargv, 0, sizeof(char*)*PARAMETER);

			myargc = 0;
			if (fp == NULL || fclose(fp) == -1 || !fwrite(buf, strlen(buf), 1, fp))
			{
				printerror(errno);
				return;
			}
...
int main(int argc, char *argv[])
{
	...
	while (1) 
	{
		...
		do
		{
			...
			else if (ch == 9 && index == length)//补全指令
			{                
